...
def clean_commute_inputs(acm_df, school_df, commute_date):
    api_key = settings.GOOGLE_MAPS_API_KEY

    # Ensure each school address has a valid location according to Google
    school_df['School_Address'] = school_df['Address'];
    del school_df['Address']
    for index, row in school_df.fillna(value='').iterrows():
        url = f"https://maps.googleapis.com/maps/api/geocode/json?address={row['School_Address']}&key={api_key}"
    ...
    if len(school_df.loc[school_df['ACM Start Time (Eastern Time)'].isnull()]) > 0:
        raise Exception(
            f"Error during commute calculation: one of your school rows is missing a start time. Each school row must have a valid start time.")

    school_df['ArrivalTime'] = commute_date + ' ' + school_df['ACM Start Time (Eastern Time)'].astype(str).str.replace(
        'AM', '').str.replace('A M', '').str.replace('A.M.', '')
    school_df['ArrivalTime'] = pd.to_datetime(school_df['ArrivalTime'])
    school_df['ArrivalTime'] = school_df['ArrivalTime'].dt.tz_localize('US/Eastern').dt.tz_convert('GMT').dt.tz_convert(
        None)
    school_df['ArrivalTime'] = (school_df['ArrivalTime'] - datetime.datetime(1970, 1, 1)).astype(
        'timedelta64[s]').astype(int)
    ...
    acm_df['tmp'] = 1
    school_df['tmp'] = 1
    commute_schl_df = acm_df.merge(school_df, on='tmp');
    del commute_schl_df['tmp']
...
def gmapsdistance(origin, destination, mode, arrival_time):
    api_key = settings.GOOGLE_MAPS_API_KEY
    url = f"https://maps.googleapis.com/maps/api/distancematrix/json?origins={origin}&destinations={destination}&mode={mode}&units=imperial&arrival_time={arrival_time}&avoid=tolls&key={api_key}"
...
def commute_procedure(commute_schl_df):
    commutes_list = []
    ...
    commutes_df = commute_schl_df[['Full.Name', 'School', 'Travel.Method', 'id_dest']].merge(commutes_df, on='id_dest',
                                                                                             how='right')
    ...
    return commutes_df[
        ['Full.Name', 'School', 'Home.Address', 'School.Address', 'Travel.Method', 'Time.Mins', 'Distance.Miles',
         'Rank', 'Status']].to_csv(index=False)
