...
school_data_file_path = args[1]
acm_survey_data_file_path = args[2]
params_file_path = args[3]
commutes_reference_file_path = args[4]
output_path = args[5]
r_scripts_path = args[6]

# writeLines(school_data_file_path)
# writeLines(acm_survey_data_file_path)
# writeLines(params_file_path)
# writeLines(commutes_reference_file_path)
# writeLines(output_path)
# writeLines(r_scripts_path)
# quit()
...
# Load Functions
cleaning_path = paste(r_scripts_path, "cleaning.R", sep="/")
placement_algo_path = paste(r_scripts_path, "placement_algorithm.R", sep="/")
source(cleaning_path)
source(placement_algo_path)

# Load Parameters
dataset <- read.csv(params_file_path, stringsAsFactors=FALSE)
dataset$acm_input_path <- acm_survey_data_file_path
dataset$sch_input_path <- school_data_file_path
...
if(score_factors$commute_factor > 0){
  acm_commutes <- read.csv(commutes_reference_file_path, check.names=FALSE)
